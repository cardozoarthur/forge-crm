...
- `crm.operational-cockpit` exposes a Forge TUI dashboard view with permission-gated CRM actions for operating snapshot refresh, relationship profile enrichment, commercial forecast review, channel intake normalization, SLA triage, segment building, campaign automation, landing page publishing, document generation, document library versioning, cross-domain work queue orchestration, design-system generation, operating copilot, specialized area copilots, enterprise customer journey execution and readiness packaging.
